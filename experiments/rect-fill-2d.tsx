...
const RectFillVisualizer = () => {
  ...
  const distanceToRect = (px, py, rect) => {
    ...
    return Math.min(...edges.map(([x1, y1, x2, y2]) => 
  ...
  const expandRect = (startX, startY, gridSize, maxRatio, existingRects = []) => {
    ...
    for (const strategy of strategies) {
      ...
      if (rect.x < outerBorder.x || rect.y < outerBorder.y ||
      ...
      let hasOverlap = obstacles.some(obs => rectOverlaps(rect, obs)) ||
      ...
      // Expand in each direction to maximum
      let improved = true;
      ...
      while (improved) {
        ...
        if (maxRight > 0) {
          ...
          for (let expand = 1; expand <= maxRight; expand++) {
            ...
            if (maxRatio !== null && maxRatio !== undefined) {
              const ratio = Math.max(testRect.width / testRect.height, testRect.height / testRect.width);
              if (ratio > maxRatio) break;
            }
            
            // Check if start point still contained
            if (startX > testRect.x + testRect.width) break;
          ...
          if (bestExpansion > 0) {
            rect = { ...rect, width: rect.width + bestExpansion };
            improved = true;
          }
        }
        
        // Try expanding DOWN
        const maxDown = outerBorder.y + outerBorder.height - (rect.y + rect.height);
        if (maxDown > 0) {
          let bestExpansion = 0;
          ...
          for (let expand = 1; expand <= maxDown; expand++) {
            ...
            // Check if start point still contained
            if (startY > testRect.y + testRect.height) break;
          ...
          if (bestExpansion > 0) {
            rect = { ...rect, height: rect.height + bestExpansion };
            improved = true;
          }
        }
        ...
        if (maxLeft > 0) {
          ...
          for (let expand = 1; expand <= maxLeft; expand++) {
            let testRect = { x: rect.x - expand, y: rect.y, width: rect.width + expand, height: rect.height };
          ...
          if (bestExpansion > 0) {
            rect = { x: rect.x - bestExpansion, y: rect.y, width: rect.width + bestExpansion, height: rect.height };
            improved = true;
          }
        }
        ...
        if (maxUp > 0) {
          ...
          for (let expand = 1; expand <= maxUp; expand++) {
            let testRect = { x: rect.x, y: rect.y - expand, width: rect.width, height: rect.height + expand };
          ...
          if (bestExpansion > 0) {
            rect = { x: rect.x, y: rect.y - bestExpansion, width: rect.width, height: rect.height + bestExpansion };
            improved = true;
          }
        }
      }
  ...
  const computeCandidatePoints = (gridSize, existingRects = []) => {
    ...
    for (let x = outerBorder.x; x < outerBorder.x + outerBorder.width; x += gridSize) {
      for (let y = outerBorder.y; y < outerBorder.y + outerBorder.height; y += gridSize) {
        // Skip points along the border edges
        if (x === outerBorder.x || y === outerBorder.y ||
        ...
        for (const blocker of allBlockers) {
          if (isPointInRect(x, y, blocker)) {
            ...
            const skipAmount = Math.max(0, Math.floor(remainingHeight / gridSize));
            if (skipAmount > 0) {
  ...
  const step = () => {
    ...
    if (expansionPhase) {
      ...
      const expandedRect = expandRect(rectToExpand.x, rectToExpand.y, currentGridSize, null, otherRects);
      ...
      const newRect = expandRect(point.x, point.y, currentGridSize, maxRatio, fillRects);
      ...
      if (newRect) {
        ...
        const remainingCandidates = candidatePoints.filter(p => 
          !isPointInRect(p.x, p.y, newRect)
        );
  ...
  const runAll = () => {
    ...
    for (let gridIdx = currentGridIndex; gridIdx < GRID_PROGRESSION.length; gridIdx++) {
    ...
    for (let i = 0; i < rects.length; i++) {
      ...
      const expandedRect = expandRect(rectToExpand.x, rectToExpand.y, finalGridSize, null, otherRects);
      if (expandedRect) {
  ...
  useEffect(() => {
    ...
    ctx.lineWidth = 3;
    ctx.strokeRect(outerBorder.x, outerBorder.y, outerBorder.width, outerBorder.height);
  ...
  }, [fillRects, currentStep, candidatePoints, expansionPhase, expansionIndex, currentGridSize]);

  return (
    <div className="w-full h-full bg-gray-900 text-white p-6">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold mb-2">Space-Filling Rectangle Algorithm</h1>
        <p className="text-gray-400 mb-6">
          Fills space with rectangles while avoiding obstacles using progressive grid refinement (100px → 50px → 20px). 
          Points furthest from obstacles are prioritized. After all grids are processed, rectangles expand without ratio constraints to eliminate seams.
        </p>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={runAll}
            disabled={(candidatePoints.length === 0 && expansionPhase && expansionIndex >= fillRects.length) || isRunning}
          ...
          <div className="ml-auto bg-gray-800 px-4 py-2 rounded-lg">
            ...
            <span className="ml-4 text-gray-400">Phase:</span> {expansionPhase ? 'Expansion' : `Candidates (${currentGridIndex + 1}/${GRID_PROGRESSION.length})`}
            {!expansionPhase && <><span className="ml-4 text-gray-400">Remaining:</span> {candidatePoints.length}</>}
            {expansionPhase && <><span className="ml-4 text-gray-400">Expanding:</span> {expansionIndex}/{fillRects.length}</>}
            <span className="ml-4 text-gray-400">Rectangles:</span> {fillRects.length}
          </div>
        </div>
