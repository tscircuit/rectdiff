...
import srj from "test-assets/arduino-uno-bottom-inner2-power-ground-pours.json"
import {
  getBounds,
...
test("Arduino UNO bottom inner2 pours mesh layer snapshots", async () => {
  ...
  for (const key of rectsByCombo.keys()) {
    const layers = key
      .split(",")
      .map((value) => Number.parseInt(value, 10))
      .filter((value) => !Number.isNaN(value))

    for (const layer of layers) layerSet.add(layer)
  }
  ...
  for (const z of layers) {
    ...
    for (const [key, rects] of rectsByCombo) {
      const layerValues = key
        .split(",")
        .map((value) => Number.parseInt(value, 10))
        .filter((value) => !Number.isNaN(value))

      if (layerValues.includes(z)) {
    ...
    const graphics: GraphicsObject = {
      title: `Arduino UNO bottom inner2 pours - z${z}`,
      texts: [
        {
  ...
  await expect(svg).toMatchSvgSnapshot(import.meta.filename)
})
