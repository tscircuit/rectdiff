...
export function makeBoardBoundsSnapshot(nodes: CapacityMeshNode[]): string {
  ...
  const rect = (cx: number, cy: number, width: number, height: number, color: string) =>
    `<rect x="${x(cx - width / 2)}" y="${y(cy + height / 2)}" width="${width * 27}" height="${height * 27}" fill="${color}" stroke="${color}" fill-opacity="0.18"/>`
  const generated = nodes.filter((node) => node.capacityMeshNodeId.startsWith("new-"))
  const outside = generated.filter((node) => node.center.y - node.height / 2 < srj.bounds.minY - 1e-8)
  const lowerEdge = Math.min(...generated.map((node) => node.center.y - node.height / 2))
...
${nodes.filter((n) => n.availableZ.includes(0)).map((n) => rect(n.center.x,n.center.y,n.width,n.height,n.capacityMeshNodeId.startsWith("new-") ? outside.includes(n) ? "#dc2626" : "#16803c" : "#64748b")).join("\n")}
